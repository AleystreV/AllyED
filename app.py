...
with st.sidebar:
    ...
    juice = st.selectbox("How often did you drink fruit juice during the past 7 days?", ["0/None", "1-3", "4-6", "1/day", "2/day", "3/day", "4+/day"], index=0)
    fruit = st.selectbox("How often did you eat fruits during the past 7 days?", ["0/None", "1-3", "4-6", "1/day", "2/day", "3/day", "4+/day"], index=1)
    salad = st.selectbox("How often did you eat salad during the past 7 days?", ["0/None", "1-3", "4-6", "1/day", "2/day", "3/day", "4+/day"], index=0)
    potato = st.selectbox("How often did you eat potato during the past 7 days?", ["0/None", "1-3", "4-6", "1/day", "2/day", "3/day", "4+/day"], index=1)
    carrot = st.selectbox("How often did you eat carrots during the past 7 days?", ["0/None", "1-3", "4-6", "1/day", "2/day", "3/day", "4+/day"], index=0)
    otherVeg = st.selectbox("How often did you eat other vegetables during the past 7 days?", ["0/None", "1-3", "4-6", "1/day", "2/day", "3/day", "4+/day"], index=1)
    soda = st.selectbox("How often did you drink soda during the past 7 days?", ["0/None", "1-3", "4-6", "1/day", "2/day", "3/day", "4+/day"], index=1)
    milk = st.selectbox("How often did you drink milk during the past 7 days?", ["0/None", "1-3", "4-6", "1/day", "2/day", "3/day", "4+/day"], index=0)
    breakfastCount = st.selectbox("How many days did you have breakfast during the past 7 days?", ["0/None", "1 day", "2 days", "3 days", "4 days", "5 days", "6 days", "7 days"], index=7)
    physicalCount = st.selectbox("How many days were you physically active during the past 7 days? (at least 60 minutes per day)", ["0/None", "1 day", "2 days", "3 days", "4 days", "5 days", "6 days", "7 days"], index=7)
    screenTime = st.selectbox("What’s your average daily screen time in hours? (Do NOT count time spent doing schoolwork.)", ["0/None", "1", "2", "3", "4", "5+"], index=5)
    peClassCount = st.selectbox("How many days did you attend PE class during the past 7 days?", ["0/None", "1 day", "2 days", "3 days", "4 days", "5 days", "6 days", "7 days"], index=0)
    sportsTeams = st.selectbox("How many sports teams did you play during the past 12 months?", ["0/None", "1 team", "2 teams", "3+ teams"], index=0)
    sleepHours = st.selectbox("On an average school night, how many hours of sleep do you get?", ["4 or less", "5", "6", "7", "8", "9", "10+"], index=3) 
    drankSportsDrink = st.selectbox("How many times did you drink a sport drink during the past 7 days?", ["0/None", "1-3", "4-6", "1/day", "2/day", "3/day", "4/day"], index=0)
    drankWater = st.selectbox("How many times did you drink a glass of water during the past 7 days?", ["0/None", "1-3", "4-6", "1/day", "2/day", "3/day", "4+/day"], index=6)
...
if file != None:
    ...
    for index, row in df.iterrows():
        input_data = preprocess_input(row["age"], row["grade"], row["height"], row["weight"], row["sexuality"], row["juice"], row["fruit"], row["salad"], row["potato"], row["carrot"], row["otherVeg"], row["soda"],
                                      row["milk"], row["breakfastCount"], row["physicalCount"], row["screenTime"], row["peClassCount"], row["sportsTeams"], str(row["sleepHours"]),
                                      row["drankSportsDrink"], row["drankWater"], row["schoolmateCloseness"], row["familyAwareness"])
